        ...
        >
          ...
          >
            <Radio.Group onChange={(e) => handleCopyModeChange(e.target.value)}>
              ...
              <Radio value="FUND_RATIO">{t('copyTradingAdd.fundRatioMode') || '资金比例模式'}</Radio>
            </Radio.Group>
          </Form.Item>
          ...
          {copyMode === 'FUND_RATIO' && (
            <div style={{ marginBottom: '16px', padding: '12px', backgroundColor: '#e6f7ff', borderRadius: '4px', border: '1px solid #91d5ff' }}>
              <div style={{ marginBottom: '8px', fontWeight: 500 }}>
                {t('copyTradingAdd.fundRatioHelp') || '💡 资金比例模式说明'}
              </div>
              <div style={{ fontSize: '13px', color: '#666' }}>
                {t('copyTradingAdd.fundRatioDesc') || '跟单金额 = 跟单比例 × (Leader开仓金额 / Leader总余额) × 你的可用余额'}
              </div>
              <div style={{ fontSize: '12px', color: '#999', marginTop: '4px' }}>
                {t('copyTradingAdd.fundRatioExample') || '例如：Leader 余额 1000 USDC，开仓 100 USDC（10%），你的余额 100 USDC，跟单比例 100%，则你跟单 10 USDC'}
              </div>
            </div>
          )}
          ...
          {(copyMode === 'RATIO' || copyMode === 'FUND_RATIO') && (
            <Form.Item
              label={copyMode === 'FUND_RATIO' 
                ? (t('copyTradingAdd.fundCopyRatio') || '跟单比例')
                : (t('copyTradingAdd.copyRatio') || '跟单比例')}
              name="copyRatio"
              tooltip={copyMode === 'FUND_RATIO'
                ? (t('copyTradingAdd.fundCopyRatioTooltip') || '100% 表示完全复制 Leader 的仓位占比，200% 表示 2 倍')
                : (t('copyTradingAdd.copyRatioTooltip') || '跟单比例表示跟单金额相对于 Leader 订单金额的百分比。例如：100% 表示 1:1 跟单，50% 表示半仓跟单，200% 表示双倍跟单')}
            >
              <InputNumber
                min={0.01}
                max={10000}
                    ...
                    validator: (_, value) => {
                      ...
                      if (typeof value === 'number' && value < minValue) {
                        const errorMsg = copyMode === 'FUND_RATIO'
                          ? (t('copyTradingAdd.minCopyAmountMin') || '最小跟单金额必须 >= 0.01')
                          : (t('copyTradingAdd.minOrderSizeMin') || '最小金额必须 >= 1')
              ...
              >
                <InputNumber
                  ...
                  placeholder={copyMode === 'FUND_RATIO'
                    ? (t('copyTradingAdd.minCopyAmountPlaceholder') || '最小跟单金额，>= 0.01（可选）')
                    : (t('copyTradingAdd.minOrderSizePlaceholder') || '仅在比例模式下生效，必须 >= 1（可选）')}
                  formatter={(value) => {
                    if (!value && value !== 0) return ''
                    const num = parseFloat(value.toString())
                    if (isNaN(num)) return ''
